...
func (s *Stream) Extract() *Message {
	msg := &Message{
		props:   s.Unpack(),
		subtag:  make([]MetaData, 0),
		recips:  make([]MetaData, 0),
		attachs: make([]MetaData, 0),
	}

	for _, maps := range s.subtag {
		msg.subtag = append(msg.subtag, maps.Unpack())
	}

	for _, maps := range s.recips {
		msg.recips = append(msg.recips, maps.Unpack())
	}

	for _, maps := range s.attachs {
		msg.attachs = append(msg.attachs, maps.Unpack())
	}

	return msg
}

func (m *Stream) Unpack() MetaData {
	var (
...
func (m *Stream) PropsNameType(entry *mscfb.File) (property_name, property_type string) {
...
var PROPS_ID_MAP = map[string]map[string]string{
